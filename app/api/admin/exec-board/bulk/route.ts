  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        if (!item.name || !item.position || !item.description) {
          errors.push(`Row ${i + 1}: Missing required fields (name, position, description)`)
          continue
        }
        ...
        const execMember = await prisma.execBoardMember.create({
          data: {
            ...
            coffeeChat: item.coffeeChat || null,
            imageUrl: imageUrl,
          },
        })

        results.push(execMember)
      } catch (error) {
