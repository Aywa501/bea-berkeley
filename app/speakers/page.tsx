  ...
  useEffect(() => {
    const fetchSpeakers = async () => {
      try {
        ...
        const cacheExpiryKey = "speakers_expiry"
        const now = Date.now()
        ...
        if (cachedData && cachedExpiry && now < Number(cachedExpiry)) {
          ...
          localStorage.setItem(cacheKey, JSON.stringify(data.speakers || []))
    ...
    <main className="flex min-h-screen flex-col">
      ...
      {selectedSpeaker && (
        <DetailModal
          ...
          image={selectedSpeaker.image ? `/images/speakers/${selectedSpeaker.image}` : undefined}
        />
      )}
    </main>
  )
