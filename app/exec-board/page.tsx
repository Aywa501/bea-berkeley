...
export default function ExecBoardPage() {
  ...
  useEffect(() => {
    const fetchExecBoard = async () => {
      try {
        const cacheKey = "exec_members"
        const cacheExpiryKey = "exec_members_expiry"
        const now = Date.now()
        ...
        if (cachedData && cachedExpiry && now < Number(cachedExpiry)) {
          // Use cached data
          setExecMembers(JSON.parse(cachedData))
        } else {
          // Fetch fresh data
          ...
          localStorage.setItem(cacheKey, JSON.stringify(data.execMembers || []))
    ...
    <main className="flex min-h-screen flex-col">
                ...
                >
                  <div className="aspect-square relative bg-gray-100">
                    {imageError[index] ? (
                      ...
                      <Image
                        src={member.image ? `/images/exec/${member.image}` : "/placeholder.svg?height=400&width=400"}
                        alt={member.name}
                        fill
      ...
      {selectedMember && (
        <DetailModal
          ...
          image={selectedMember.image ? `/images/exec/${selectedMember.image}` : undefined}
          links={{
            linkedin: selectedMember.linkedin,
            coffeeChat: selectedMember.coffeeChat,
          }}
        />
      )}
    </main>
